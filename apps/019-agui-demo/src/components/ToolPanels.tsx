...
function parseSendMailToolInputPartial(input: JsonValue | undefined): Partial<SendMailToolInput> | undefined {
...
function WebSearchToolPanel({ input, output }: { input?: WebSearchToolInput; output: WebSearchToolOutput }) {
      ...
      <div className='tool-panel__head'>
        ...
        {query ? (
          <span className='tool-panel__query'>
            「{query}」{count != null ? <span className='tool-panel__count'> · {count} 条</span> : null}
          </span>
        ) : null}
      </div>
      {items.length > 0 ? (
...
function ToolPendingPanel({ name, hint }: { name: string; hint?: string }) {
  ...
  return (
    <div className={`tool-panel tool-panel--pending${isMail ? ' tool-panel--pending-mail' : ''}`} aria-busy='true'>
...
function DefaultToolOutput({ value }: { value: JsonValue | WebSearchToolOutput }) {
  const text = typeof value === 'string' ? formatDefaultToolString(value) : JSON.stringify(value, null, 2);
...
function ToolMessagePart({ part }: { part: AnyToolPart }) {
  ...
  switch (name) {
    case 'web_search': {
      ...
      const output = normalizeWebSearchOutput(outputJson !== undefined ? outputJson : String(rawOut));
