...
import type { SearchResultData } from '@zilliz/milvus2-sdk-node';
import { MetricType, MilvusClient } from '@zilliz/milvus2-sdk-node';
import { embeddings, modelWithTemperature } from '../model';
import { COLLECTION_NAME } from '../vector-db';

// 初始化 OpenAI Chat 模型
const model = modelWithTemperature;
...
const milvusSearch = new RunnableLambda<
  ...
  { question: string; retrievedContent: SearchResultData[] }
>({
  func: async (input: { question: string; k?: number }) => {
...
const buildPromptInput = new RunnableLambda<
  { question: string; retrievedContent: SearchResultData[] },
  { hasContext: boolean; question: string; context: string; retrievedContent: SearchResultData[] }
>({
  func: async (input: { question: string; retrievedContent: SearchResultData[] }) => {
...
const ragChain = RunnableSequence.from([
  ...
  buildPromptInput,
  new RunnableLambda<
    { hasContext: boolean; question: string; context: string },
    { question: string; context: string; answer?: string; noContext: boolean }
  >({
    func: async (input: { hasContext: boolean; question: string; context: string }) => {
