 ...
 * 重建 ES 索引并 bulk 写入
 */
async function seedElasticsearch(indexName: string, rows: typeof ROWS) {
  try {
    console.log('\n[Elasticsearch]');
    ...
    console.error('Elasticsearch 出错:', (error as Error).message);
    throw error;
  }
}
 ...
 */
async function seedMilvus(collectionName: string, rows: typeof ROWS, emb: OpenAIEmbeddings) {
  try {
    console.log('\n[Milvus]');
    ...
    console.error('Milvus 出错:', (error as Error).message);
    throw error;
  }
}
