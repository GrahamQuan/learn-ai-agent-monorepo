...
import { DashScopeRerank } from "../rerank/dashscope-rerank.js";
import {
  augmentQuery,
  retrievalQueryStrings,
} from "./query-augment.js";

const INDEX = "life_notes";

const HybridRetrievalState = Annotation.Root({
  query: Annotation<string>(),
  queryAugmentation: Annotation<Awaited<ReturnType<typeof augmentQuery>>>(),
  esHits: Annotation<Document[]>(),
  milvusHits: Annotation<Document[]>(),
  merged: Annotation<Document[]>(),
  topDocuments: Annotation<Document[]>(),
  answer: Annotation<string>(),
});

function docFromEsHit(hit: estypes.SearchHit<Record<string, unknown>>) {
  const s = hit._source ?? {};
...
function merge(esDocs: Document[], milvusDocs: Document[]) {
...
function dedupeDocsById(docs: Document[]) {
  const seen = new Set();
  const out = [];
  for (const d of docs ?? []) {
...
function printDocs(label: string, docs: Document[]) {
...
function printQueryRewrite(original: string, augmentation: Awaited<ReturnType<typeof augmentQuery>>) {
...
function stringifyMessageContent(content: MessageContent) {
...
function formatDocsAsContext(docs: Document[]) {
  return (docs ?? [])
    .map((d, i) => {
...
export function compileHybridRetrievalGraph(esClient: Client, milvus: Milvus, reranker: DashScopeRerank, chatModel: ChatOpenAI) {
  const ES_K = 15;
  const MILVUS_K = 15;
