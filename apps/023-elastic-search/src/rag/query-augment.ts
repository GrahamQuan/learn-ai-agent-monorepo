...
function normalizeThreeQueries(original: string, list: string[]) {
  const out = (list ?? [])
...
export async function augmentQuery(chatModel: ChatOpenAI, query: string) {
...
export function retrievalQueryStrings(original: string, augmentation?: z.infer<typeof QueryAugmentationSchema>) {
