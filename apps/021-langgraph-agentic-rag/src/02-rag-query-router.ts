...
const ragGenerateNode = async (state: typeof GraphState.State) => {
...
function decideNext(state: typeof GraphState.State) {
...
async function main() {
  const question = "雁门关事件的主谋，他的儿子最终结局是什么？"; // (1) rag_generate question
  // const question = "意面的做法？"; // (2) direct_answer
  const k = 5;
