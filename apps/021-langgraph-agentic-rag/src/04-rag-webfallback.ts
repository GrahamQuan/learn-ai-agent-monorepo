...
const GraphState = Annotation.Root({
  ...
  >(),
  localContext: Annotation<string>(),
...
async function retrieveRelevantContent(query: string, k: number) {
  try {
...
const routeQuestionNode = async (state: typeof GraphState.State) => {
...
const directAnswerNode = async (state: typeof GraphState.State) => {
...
const retrieveLocalNode = async (state: typeof GraphState.State) => {
...
const evaluateNode = async (state: typeof GraphState.State) => {
...
async function bochaWebSearch(query: string, count?: number) {
  ...
  try {
    ...
    throw new Error(`搜索 API 请求失败（网络错误）：${(error as Error).message}`);
  }
  ...
  try {
    ...
    throw new Error(`搜索结果解析失败：${(error as Error).message}`);
  }
...
function afterRoute(state: typeof GraphState.State) {
  return state.strategy === "simple" ? "direct_answer" : "local_retrieve";
}

function afterEvaluateLocal(state: typeof GraphState.State) {
...
async function main() {
  ...
  try {
    ...
    if (!(error as Error).message.includes("already loaded")) throw error;
