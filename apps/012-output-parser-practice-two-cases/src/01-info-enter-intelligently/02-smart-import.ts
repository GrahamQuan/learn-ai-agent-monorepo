...
4. 返回格式必须是一个数组，即使只有一个人也要放在数组中`;
    ...
    const insertedRows = (await sql.query(insertSql, values.flat())) as Array<{ id: number }>;
    const insertIds = insertedRows.map((row) => row.id);
    console.log(`✅ 成功批量插入 ${insertedRows.length} 条数据`);
    console.log(`   插入的ID：${insertIds.join(', ')}`);
