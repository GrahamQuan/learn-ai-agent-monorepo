...
import type { ParsedToolCall } from '@langchain/core/output_parsers/openai_tools';
import { JsonOutputToolsParser } from '@langchain/core/output_parsers/openai_tools';
import type { ChatGeneration } from '@langchain/core/outputs';
import chalk from 'chalk';
import { model } from '../model';
import { executeCommandTool, listDirectoryTool, readFileTool, writeFileTool } from './00-all-tools';

const tools = [readFileTool, writeFileTool, executeCommandTool, listDirectoryTool];
type AgentTool = (typeof tools)[number];
type InvokableAgentTool = AgentTool & {
  invoke: (args: ParsedToolCall['args']) => Promise<string>;
};
...
// Agent 执行函数
async function runAgentWithTools(query: string, maxIterations: number = 30): Promise<MessageContent> {
...
`),
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    const toolParser = new JsonOutputToolsParser<ParsedToolCall[]>();
    ...
    for await (const chunk of rawStream) {
      ...
      try {
        parsedTools = await toolParser.parseResult([{ message: fullAIMessage } as unknown as ChatGeneration]);
      } catch (_e) {
      ...
      if (parsedTools && parsedTools.length > 0) {
        for (const toolCall of parsedTools) {
          if (toolCall.type === 'write_file' && toolCall.args?.content) {
            ...
            if (currentContent.length > (previousLength as number)) {
              const newContent = currentContent.slice(previousLength as number);
    ...
    const completeAIMessage = fullAIMessage as AIMessageChunk;

    await history.addMessage(completeAIMessage);
    console.log(chalk.green('\n✅ 消息已完整存入历史'));

    // 检查是否有工具调用
    if (!completeAIMessage.tool_calls || completeAIMessage.tool_calls.length === 0) {
      console.log(`\n✨ AI 最终回复:\n${completeAIMessage.content}\n`);
      return completeAIMessage.content;
    }
    ...
    for (const toolCall of completeAIMessage.tool_calls) {
      ...
      if (foundTool) {
        const toolResult = await (foundTool as InvokableAgentTool).invoke(toolCall.args);
        await history.addMessage(
          new ToolMessage({
            content: toolResult,
