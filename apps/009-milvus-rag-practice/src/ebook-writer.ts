 ...
 * 创建或获取集合
 */
async function ensureCollection(bookId: number): Promise<void> {
  try {
    // 检查集合是否存在
 ...
 */
async function insertChunksBatch(chunks: string[], bookId: number, chapterNum: number): Promise<number> {
  try {
    if (chunks.length === 0) {
      return 0;
    }
    ...
    console.error(`插入章节 ${chapterNum} 的数据时出错:`, (error as Error).message);
 ...
 */
async function loadAndProcessEPubStreaming(bookId: number): Promise<number> {
  try {
