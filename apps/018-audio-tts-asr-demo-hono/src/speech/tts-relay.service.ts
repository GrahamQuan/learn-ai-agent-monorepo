...
export class TtsRelayService {
  private readonly logger = {
    log: (message: string) => console.log(`[TtsRelayService] ${message}`),
  };
  ...
  constructor(configService: ENV) {
    this.secretId = configService.SECRET_ID;
    this.secretKey = configService.SECRET_KEY;
    this.appId = configService.APP_ID;
    this.voiceType = configService.TTS_VOICE_TYPE ?? 101001;
  }
  ...
  handleAiStreamEvent(event: AiTtsStreamEvent): void {
    ...
    switch (event.type) {
      ...
      case 'end': {
        ...
        this.completeTencentSessionIfReady(session);
        break;
      }
      case 'error': {
        this.sendClientJson(session.clientWs, {
  ...
  private completeTencentSessionIfReady(session: ClientSession): void {
    if (
