...
const SECRET_ID = env.AUDIO_SECRET_ID;
const SECRET_KEY = env.AUDIO_SECRET_KEY;
const APP_ID = env.AUDIO_APP_ID;
...
async function sendTexts(ws: WebSocket, sessionId: string) {
...
function streamTTS() {
  ...
  ws.on('message', async (data, isBinary) => {
    ...
    try {
      ...
      console.error('[解析错误]', (e as Error).message);
    }
  });
