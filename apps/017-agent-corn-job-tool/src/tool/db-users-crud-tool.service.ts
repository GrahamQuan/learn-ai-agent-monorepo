  ...
  constructor(private readonly usersService: UsersService) {
    ...
    this.tool = tool(
      async ({ action, id, name, email }) => {
        switch (action) {
          case 'create': {
            if (!name || !email) return '创建用户需要同时提供 name 和 email。';
          ...
          case 'list': {
            ...
            if (!users.length) return '数据库中还没有任何用户记录。';
            const lines = users
              .map(
                (user) =>
          ...
          case 'update': {
            ...
            if (!existing) return `ID 为 ${id} 的用户在数据库中不存在。`;
          ...
          case 'delete': {
            if (!id) return '删除用户需要提供 id。';
            const existing = await this.usersService.findOne(id);
            if (!existing) return `ID 为 ${id} 的用户在数据库中不存在，无需删除。`;
