...
注意：像“\`1分钟后提醒我喝水\`”，时间相关信息用于计算下一次执行时间，而 \`instruction\` 应该是“提醒我喝水”；本轮不需要立刻提醒。`,
    ...
    while (true) {
      ...
      if (!toolCalls.length) return aiMessage.content as string;
      ...
      for (const toolCall of toolCalls) {
        ...
        if (toolName === 'query_user') result = await this.queryUserTool.invoke(toolCall.args);
        else if (toolName === 'send_mail') result = await this.sendMailTool.invoke(toolCall.args);
        ...
        else if (toolName === 'cron_job') result = await this.cronJobTool.invoke(toolCall.args);
...
- 给出 Cron 表达式 => \`cron_job.type=cron\``,
    ...
    while (true) {
      ...
      for await (const chunk of stream) {
        ...
        if (!isToolCalling && chunk.content) yield chunk.content as string;
      }
