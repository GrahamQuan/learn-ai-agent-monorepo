 ...
 * 从 Milvus 中检索相关的历史对话
 */
async function retrieveRelevantConversations(query: string, k: number = 2) {
  try {
    // 生成查询的向量
    const queryVector = await getEmbedding(query);

    // 在 Milvus 中搜索相似的对话
    const searchResult = await vectorDatabaseClient.search({
...
async function retrievalMemoryDemo() {
  ...
  const conversations: { input: string }[] = [
  ...
  for (let i = 0; i < conversations.length; i++) {
    const { input } = conversations[i] as { input: string };
...
${conv.content}`;
    ...
    try {
      await vectorDatabaseClient.insert({
        collection_name: COLLECTION_NAME,
        data: [
          {
            id: convId,
