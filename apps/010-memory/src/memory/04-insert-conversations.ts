...
import { embeddings, VECTOR_DIM } from '../model';
import { COLLECTION_NAME, vectorDatabaseClient } from '../vector-db';
...
async function main() {
  try {
    ...
    await vectorDatabaseClient.connectPromise;
    console.log('✓ 已连接\n');

    // 创建集合
    console.log('创建集合...');
    await vectorDatabaseClient.createCollection({
    ...
    // 创建索引
    console.log('\n创建索引...');
    await vectorDatabaseClient.createIndex({
      collection_name: COLLECTION_NAME,
      field_name: 'vector',
    ...
    // 加载集合
    console.log('\n加载集合...');
    await vectorDatabaseClient.loadCollection({ collection_name: COLLECTION_NAME });
    console.log('✓ 集合已加载');

    // 插入对话数据
    console.log('\n插入对话数据...');
    const conversations: { id: string; content: string; round: number; timestamp: string }[] = [
      {
        id: 'conv_001',
    ...
    const insertResult = await vectorDatabaseClient.insert({
      collection_name: COLLECTION_NAME,
