...
const state = {
  messages: {
    value: (left: BaseMessage[], right: BaseMessage | BaseMessage[]) =>
...
function userQuery(state: { messages: BaseMessage[] }) {
...
// 步骤1：生成 Cypher
// ----------------------
async function generateCypher(state: { messages: BaseMessage[] }) {
    const prompt = `
    ...
    return { cypher: res.content as string }
  }

// ----------------------
// 步骤2：执行图查询
// ----------------------
async function executeGraphQuery(state: { cypher: string }) {
  try {
...
// 步骤3：生成答案
// ----------------------
async function generateAnswer(state: { messages: BaseMessage[]; context: string }) {
  const prompt = `
...
// 运行 GraphRAG
// ----------------------
async function runGraphRAG(question: string) {
